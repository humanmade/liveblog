...
var _class = function () {
	...
	_createClass(_class, [{
		...
		key: 'hasRequestToken',
		value: function hasRequestToken() {
		...
		value: function authorize(next) {
			...
			if (savedTemporaryCredentials) {
				...
				if (args.oauth_token) {
					...
					var ret = this.getAccessToken(args.oauth_verifier).then(function (token) {
						_this4.saveCredentials();
						return token;
					});
		...
		value: function request(method, url) {
			...
			var tokenlessURLs = [this.config.url + 'oauth1/request', this.config.brokerURL + 'broker/connect'];
			...
			if (this.hasCredentials() || tokenlessURLs.indexOf(url) > -1) {
				var oauth1 = new _oauth2.default({
			...
			if (oauth1) {
				headers = _extends({}, headers, oauth1.toHeader(oauthData));
			}
